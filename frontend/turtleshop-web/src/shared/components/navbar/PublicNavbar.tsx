    ...
    const loadCounts = async () => {
      try {
        ...
        const cartProductIds = Array.from(
          new Set(activeCartItems.map((item: any) => item.productId).filter(Boolean))
        );
    ...
    <header className="site-header">
      ...
      <div className="nav-links-wrapper">
        <nav className="nav-links">
          ...
          <NavLink to="/wishlist" className={navClass}>
            Wishlist
            {wishlistCount > 0 && <span className="badge badge-nav">{wishlistCount}</span>}
          </NavLink>
        </nav>
      ...
      <div className="auth-actions">
        ...
        >
          ...
          {cartCount > 0 && <span className="badge badge-nav">{cartCount}</span>}
        </a>
